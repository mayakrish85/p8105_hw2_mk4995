...
``` {r}
# Count subset of trash wheel dataset
n_mr_trash = 
  trash_wheel_df |> 
...
# Bakers_df does not contain episodes column.
...
```{r}
# Using anti-join
bakers_not_in_results <- anti_join(bakers_df, results_df, by = c("series", "episode", "baker"))
bakers_not_in_bakes <- anti_join(bakers_df, bakes_df, by = c("series", "episode", "baker"))
```
